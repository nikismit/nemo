...
public class NemoControllerBeer : MonoBehaviour
{
    public string NemoControllerValueRaw = "E:10334 340";
    public float NemoControllerValueParsed;
    ...
    int GetValueFromBelt()
    {
        ...
        if (NemoControllerValueRaw.Contains("E")) {
            // NemoControllerValueParsed = float.Parse(NemoControllerValueRaw.Split(','));
            if (NemoControllerValueRaw.Split(',').Length == 2)
            {
                NemoControllerValueParsed = float.Parse(NemoControllerValueRaw.Split(',')[1]);
            }
